# LLM Inference & Serving

## Goal

Benchmark inference stacks for throughput, latency, and efficiency.

## Problem Space

Serving architecture, KV-cache reuse, high-throughput inference, and low-bit deployment.

## Frameworks

| # | Framework | Status | Planned mini-build | Docs Page |
|---|---|---|---|---|
| 033 | RTP-LLM | Pending | High-throughput Qwen serving benchmark with RTP-LLM. | [Experiment](../experiments/033-rtp-llm.md) |
| 034 | Mooncake | Pending | Repeated long-context workload üçün KV-cache reuse demo qur. | [Experiment](../experiments/034-mooncake.md) |
| 035 | BitNet.cpp | Pending | BitNet b1.58 modelini CPU-da run edib normal quantized model ilə müqayisə et. | [Experiment](../experiments/035-bitnet-cpp.md) |
